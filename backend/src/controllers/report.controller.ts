...
export const downloadReport = async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    ...
    const pdfPathOrUrl = await reportService.getReportPath(candidateId, userId);

    if (typeof pdfPathOrUrl === 'string' && (pdfPathOrUrl.startsWith('http://') || pdfPathOrUrl.startsWith('https://'))) {
      // Remote file (Cloudinary) — redirect to the Cloudinary URL
      return res.redirect(pdfPathOrUrl);
    }

    return res.download(pdfPathOrUrl, `bgv-report-${candidateId.slice(0, 8)}.pdf`);
  } catch (error: any) {
    if (error.status === 404) return res.status(404).json({ error: error.message });
    if (error.status === 403) return res.status(403).json({ error: error.message });
